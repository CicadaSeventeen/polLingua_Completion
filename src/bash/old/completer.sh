...
_pinyin_completion() {
    ...
    if [[ "${compgen_opts[0]}" == -d ]]; then
        readarray -t pinyin_matched < <(eval COMPLETION_FILE_TYPE=dir $_polLingua_commands "$cur"  $PWD)
    else
        readarray -t pinyin_matched < <(eval $_polLingua_commands "$cur" $PWD)
